...
use crate::ffi::{self,kqueue};
use super::{Events, Poll, Registry};

impl Poll {
    pub fn new() -> Result<Self> {
        let res = unsafe {
            kqueue::epoll_create(1) // The argument is actually ignored by epoll_create, but must be > 0
        };
    ...
    pub fn poll(&mut self, events: &mut Events, timeout: Option<i32>) -> Result<()> {
        ...
        let res = unsafe { kqueue::epoll_wait(fd, events.as_mut_ptr(), max_events, timeout) };
...
impl Registry {
    pub fn register(&self, source: &TcpStream, token: usize, interests: i32) -> Result<()> {
        ...
        let op = kqueue::EPOLL_CTL_ADD;
        let res = unsafe {
            kqueue::epoll_ctl(self.raw_fd, op, source.as_raw_fd(), &mut event)
        };
